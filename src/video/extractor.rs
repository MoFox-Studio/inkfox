...
impl VideoKeyframeExtractor {
    ...
    pub fn new(ffmpeg_path: String, threads: usize, verbose: bool) -> PyResult<Self> {
        // 防止重复设置全局线程池
        static GLOBAL_THREAD_POOL_SET: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
        if threads > 0 && GLOBAL_THREAD_POOL_SET.load(std::sync::atomic::Ordering::SeqCst) == false {
            let _ = rayon::ThreadPoolBuilder::new().num_threads(threads).build_global();
            GLOBAL_THREAD_POOL_SET.store(true, std::sync::atomic::Ordering::SeqCst);
        }
    ...
    pub fn process_video(&self, video_path: &str, output_dir: &str, max_keyframes: usize, max_save: Option<usize>, use_simd: Option<bool>, block_size: Option<usize>) -> PyResult<PyPerformanceResult> {
        ...
        // 注意：这里假设 run_performance_test 是“测试”，不执行实际保存等操作
        // 如果它执行了实际操作，会导致重复
        ...
        // 提取关键帧
        let keyframes = extract_keyframes_optimized(
            &frames,
            max_keyframes,
            use_simd,
            block_size,
    ...
    pub fn get_cpu_features(&self) -> PyResult<HashMap<String, bool>> {
        ...
        #[cfg(target_arch = "x86_64")]
        {
            ...
            f.insert("sse4_1".into(), std::arch::is_x86_feature_detected!("sse4.1"));
            f.insert("sse4_2".into(), std::arch::is_x86_feature_detected!("sse4.2"));
    ...
    pub fn get_configured_thread_count(&self) -> usize {
        self.threads
    }
