...
pub fn extract_frames_memory_stream(
    ...
    loop {
        match reader.read_exact(&mut buf) {
            Ok(()) => {
                frames.push(PyVideoFrame::new(frame_count, width, height, buf.clone()));
                frame_count += 1;
                if verbose && frame_count % 1000 == 0 {
                    vprintln!(true, "Processed {} frames", frame_count);
                }
            }
            Err(_) => break,
        }
    ...
    for line in info.lines() {
        if line.contains("Video:") && line.contains('x') {
            for part in line.split_whitespace() {
                if let Some(p) = part.find('x') {
                    let (w, h_part) = part.split_at(p);
                    let h_seg = &h_part[1..];
                    let h_str = h_seg.split(',').next().unwrap_or(h_seg);
                    if let (Ok(wu), Ok(hu)) = (w.parse(), h_str.parse()) {
                        return Some((wu, hu));
                    }
                }
            }
...
pub fn extract_keyframes_optimized(
    ...
    diffs.select_nth_unstable_by(k_index, |a, b| {
...
pub fn run_performance_test(
    ...
    let keyframes =
        extract_keyframes_optimized(&frames, max_keyframes, use_simd, block_size, verbose)?;
