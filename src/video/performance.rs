...
impl PyPerformanceResult {
    fn to_dict(&self) -> PyResult<HashMap<String, PyObject>> {
        Python::with_gil(|py| {
            ...
            d.insert(
                "frame_extraction_time_ms".into(),
                self.frame_extraction_time_ms.to_object(py),
            );
            d.insert(
                "keyframe_analysis_time_ms".into(),
                self.keyframe_analysis_time_ms.to_object(py),
            );
            d.insert("total_frames".into(), self.total_frames.to_object(py));
            d.insert(
                "keyframes_extracted".into(),
                self.keyframes_extracted.to_object(py),
            );
            ...
            d.insert(
                "max_keyframes_requested".into(),
                self.max_keyframes_requested.to_object(py),
            );
            d.insert(
                "optimization_type".into(),
                self.optimization_type.to_object(py),
            );
