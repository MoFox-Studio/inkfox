...
impl PyVideoFrame {
    pub fn calculate_difference_parallel_simd(
        &self,
        other: &PyVideoFrame,
        block_size: usize,
        use_simd: bool,
        ...
        let total_diff: u64 = (0..num_blocks)
            ...
            .map(|i| {
                ...
                let end = ((i + 1) * block_size).min(total_pixels);
                let block_len = end - start;
                if use_simd {
                    #[cfg(target_arch = "x86_64")]
                    unsafe {
                        if std::arch::is_x86_feature_detected!("avx2") {
                            return self.calculate_difference_avx2_block(
                                &other.data,
                                start,
                                block_len,
                            );
                        } else if std::arch::is_x86_feature_detected!("sse2") {
                            return self.calculate_difference_sse2_block(
                                &other.data,
                                start,
                                block_len,
                            );
    ...
    unsafe fn calculate_difference_avx2_block(
        &self,
        other: &[u8],
        start: usize,
        len: usize,
        ...
        for i in 0..chunks {
            ...
            total += _mm256_extract_epi64(diff, 0) as u64
        ...
        for i in (start + chunks * 32)..(start + len) {
            total += (self.data[i] as i32 - other[i] as i32).abs() as u64;
        }
        total
    }
    #[cfg(target_arch = "x86_64")]
    #[target_feature(enable = "sse2")]
    unsafe fn calculate_difference_sse2_block(
        &self,
        other: &[u8],
        start: usize,
        len: usize,
